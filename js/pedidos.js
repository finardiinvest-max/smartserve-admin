...
async function loadOrders() {
    ...
    try {
        ...
        // Filtrar pedidos entregues apenas do dia atual
        const hoje = new Date();
        hoje.setHours(0, 0, 0, 0);
        
        orders.forEach(order => {
            if (order.statusPedido === 'cancelado') return;
            
            if (order.statusPedido === 'entregue') {
                // Verificar se o pedido foi entregue hoje
                const dataEntrega = new Date(order.dataHora);
                dataEntrega.setHours(0, 0, 0, 0);
                
                if (dataEntrega.getTime() === hoje.getTime()) {
                    ordersByStatus.entregue.push(order);
                }
            } else if (ordersByStatus[order.statusPedido]) {
...
function createOrderCard(order) {
    ...
    // Extrair nome e telefone do cliente com múltiplas tentativas
    ...
    // Tentar múltiplas formas de acessar o nome
    if (order.cliente) {
        console.log('order.cliente existe:', order.cliente);
        
        // Tentativa 1: order.cliente.nome
        if (order.cliente.nome) {
            clienteNome = order.cliente.nome;
            console.log('✅ Nome encontrado em order.cliente.nome:', clienteNome);
        }
        // Tentativa 2: order.cliente.name (caso venha em inglês)
        else if (order.cliente.name) {
            clienteNome = order.cliente.name;
            console.log('✅ Nome encontrado em order.cliente.name:', clienteNome);
        }
        // Tentativa 3: verificar se cliente é string
        else if (typeof order.cliente === 'string') {
            clienteNome = order.cliente;
            console.log('✅ Cliente é string:', clienteNome);
        }
        else {
            console.log('❌ Nome NÃO encontrado. Estrutura de cliente:', Object.keys(order.cliente));
        }
        
        // Telefone
        if (order.cliente.telefone) {
            clienteTelefone = order.cliente.telefone;
        } else if (order.cliente.phone) {
            clienteTelefone = order.cliente.phone;
        }
    } 
    // Tentativa 4: campos diretos no order
    else if (order.clienteNome) {
        clienteNome = order.clienteNome;
        clienteTelefone = order.clienteTelefone || '';
        console.log('✅ Nome encontrado em order.clienteNome:', clienteNome);
    }
    else {
        console.log('❌ order.cliente NÃO existe! Chaves do order:', Object.keys(order));
    }
    
    console.log('Nome final:', clienteNome);
    console.log('Telefone final:', clienteTelefone);
    console.log('===================')
